...
public class CURSketcher {
    ...
    private double frobeniusCorrelation;
    private double[][] C;
    private double[][] U;
    private double[] R;
    private double[][] A;
    ...
    public CURSketcher(String fileName, int r, int c, int k, int seed) {
        this.r = r;
        this.c = c;
        this.k = k;
        if (k == 0)
    ...
    private void writeCURSketch(Set selectedColumns) {
        ...
        for (int i = 0; i < nRows; i++) {
            for (int j = 0; j < selectedColumns.size(); j++) {
                A[i][j] = (dataSource.maxValues[j] - dataSource.minValues[j]) * A[i][(Integer) s[j]] + dataSource.minValues[(Integer) s[j]];
                if (j < (selectedColumns.size() - 1)) {
                    writer.print(A[i][(Integer) s[j]] + ",");
                } else {
                    writer.println(A[i][(Integer) s[j]]);
                }
            }
        }
        writer.close();
        System.out.println("CURSketcher output file name is cursketch.csv");
        System.out.println("CURSketcher output number of columns " + selectedColumns.size());
    }
